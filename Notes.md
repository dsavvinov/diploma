...
Вопросы:
- У меня есть `KtElement`, я хочу проверить, не является ли он `KtCallExpression`. Видел в `checkNothingType`, что там сначала проверяют на `KtExpression`, потом делают `deparenthesize`, и только потом проверяют на конкретные типы. Надо всегда так делать?
...
- UnionTypes это ведь легальная конструкция? В том плане, что мы можем в одном месте узнать что какая-то переменная `is A`, а в другом -- `is B`, и это +- легально, т.к. может же быть класс, который наследуется и от `A` и от `B` (видимо нужно наложить условие, что они -- интерфейсы)

TODO: 
- Пока что нет дальнейшего анализа летящего исключения в try-catch блоках
- Анричбл код репортится куда-то странно
