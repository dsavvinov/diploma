...
class Evaluator : SchemaVisitor<EsNode> {
    ...
    override fun visit(effect: Effect): EsNode {
        ...
        // TODO: we can evaluate it further
                    ...
                    .forEach { (outcome, e) ->
                        if (outcome is Returns) {
                            if (outcome.value == true.lift()) {
                                ...
                                resultEffects.add(EffectImpl(e.premise, e.conclusion))
                                resultEffects.add(EffectImpl(e.premise, evalConclusion))
                            }
...
fun (EsNode).evaluate() : EsNode {
    val evaluatedSchema = Evaluator().let { accept(it) }
    return evaluatedSchema.filter { it !is Effect || (it is Effect && it.premise != false.lift()) }!!
